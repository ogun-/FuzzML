...
parser.add_argument(
    "--no-cert-validate", action="store_true", help="Disable certificate validation"
)
...
header_group.add_argument(
    "--header", metavar="<Header>", nargs="*", help="Specify required request headers"
)
header_group.add_argument(
    "--fheader",
    metavar="<Headers file>",
    help="Specify a file containing the required request headers",
)
...
data_group.add_argument(
    "--data", metavar="<POST content>", help="Data to be sent inside the request body"
)
data_group.add_argument(
    "--fdata",
    metavar="<POST content file>",
    help="Specify a file containing the data to be sent inside the request body",
)
...
def check_url_syntax(url):
    if (url.find("http://", 0, 7) == -1) and (url.find("https://", 0, 8) == -1):
            ...
            "Address not starting with http or https.\n[-] Check your URL and try again.\n"
        )
...
def get_save_filename(url, clock):
    if url.startswith("http://"):
        ...
        address = address + "_" + clock
        # www.example.com_webservice.php_20141110_1822.15.059238.xml
        return address
...
def save_data(request_content, response_content, clock):
    path = "requests/"
    # path to write requests's response
    if not os.path.exists(path):
        os.makedirs(path)
    with open(
        path + get_save_filename(args.url, clock) + "_req.xml", "w+"
    ) as fp_req:  # requests/www.example.com_webservice.php_20141110_1822.15.059238_req.xml
        fp_req.write(request_content)
        # fixing problems with unicode characters
    with open(
        path + get_save_filename(args.url, clock) + "_resp.xml", "w+"
    ) as fp_resp:  # requests/www.example.com_webservice.php_20141110_1822.15.059238_resp.xml
...
def set_req_body(cmdline_data, file_data):
    if (cmdline_data is not None) and (
        file_data is None
    ):  # if the user defined data through command line
        return cmdline_data
    elif (cmdline_data is None) and (
        file_data is not None
    ):  # if the user defined data through a file
        if os.path.exists(file_data):
            with open(file_data, "r") as f:
                body = f.read(20 * 1024)
                # read at most 20 KB from file
            return body
    ...
    if nodes_to_duplicate:
        ...
        for node in nodes_to_duplicate:
            ...
            for child in children:
                ...
                node.insert(get_children(node).index(child), child_dup)
                # places the duplicated node side-by-side with the original node
...
def fuzzml_element_omission(root, url, hr):
    ...
    if nodes_to_remove:
        ...
        for node in nodes_to_remove:
            children = get_children(node)
            for child in children:
                index = get_children(node).index(child)
                # remember element position
                node.remove(child)
                keep_information(et.tostring(tree_root), url, hr)
                node.insert(index, child)
                # places the removed node back into place
    else:
        end("Tree has only one Element\n")
...
def fuzzml_element_tag_malformation(root, url, hr):
    ...
    if nodes_to_remove:
        ...
        xml_allowed_chars = sum(
            list((list(range(65, 90)), [95], list(range(97, 122)))), []
        )
        # creating list of acceptable chars in an xml tag
        for node in nodes_to_remove:
            children = get_children(node)
            for child in children:
                if "}" in child.tag:
                    ...
                    tag = str(child.tag)
                element_position = get_children(node).index(child)
                # remember element position
                char_index = random.choice(list(range(0, len(tag))))
                # choosing a random char to strip from tag
                if namespace:
                    child.tag = namespace + "}" + tag.replace(tag[char_index], "")
                    # stripping char from tag
                else:
                    child.tag = tag.replace(tag[char_index], "")
                    # stripping char from tag
                child.tag = child.tag + chr(random.choice(xml_allowed_chars))
                # adding a random ascii char suffix
                keep_information(et.tostring(tree_root), url, hr)
                if namespace:
...
def keep_information(fuzzed_xml, url, hr):
    ...
    save_data(
        fuzzed_xml_request.toprettyxml(),
        fuzzed_xml_response.toprettyxml(),
